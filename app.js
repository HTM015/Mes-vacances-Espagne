...
function exportBackup() {
  ...
  link.download = `horizon-chill-${new Date().toISOString().slice(0, 10)}.json`;
  link.click();
...
function getGreeting() {
  const hour = new Date().getHours();
  if (hour < 12) return "Bon matin roadtrip ☀️";
  if (hour < 18) return "Prêt pour l'été ? 🌅";
  return "Ambiance coucher de soleil 🌇";
}
